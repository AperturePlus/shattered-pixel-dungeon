...
public class Badges {
	...
	public enum Badge {
		...
		/**
		 * 获取徽章的标题。
		 * 通过调用Messages类的get方法，结合当前对象的名称和".title"后缀，来获取徽章的标题。
		 * 这种设计允许通过外部配置或资源文件来动态定义徽章的标题，提高了代码的灵活性和可维护性。
		 *
		 * @return 徽章的标题字符串。
		 */
		public String title(){
		...
		/**
		 * 获取徽章的描述。
		 * 通过调用Messages类的get方法，结合当前对象的名称和".desc"后缀，来获取徽章的描述。
		 * 这种设计允许通过外部配置或资源文件来动态定义徽章的描述，提高了代码的灵活性和可维护性。
		 *
		 * @return 徽章的描述字符串。
		 */
		public String desc(){
		...
		/**
		 * Badge类的构造方法。
		 * 该构造方法调用了另一个带有整型参数的构造方法，以实现初始化过程。
		 * 使用了缺省参数值-1，这可能表示一个未指定或默认的状态，具体的含义需要结合实际业务逻辑来理解。
		 */
		Badge() {
		    this( -1 );
		}
